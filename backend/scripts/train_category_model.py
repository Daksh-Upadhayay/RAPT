...
- fits on the train split of data/processed/tickets.csv
...
def main() -> None:
    ...
    xgb = train_xgboost(make_features(), data["train"], data["val"])
    print(f"  selected {xgb.selection}")
    print("Training logistic-regression baseline ...")
    logreg = train_logreg_baseline(make_features(), data["train"])
...
test {len(splits["test"])} rows (grouped split, see `scripts/build_dataset.py`). Hand-written
...
"""
    metrics = {
        ...
        "results": results,
        "metadata": metadata(DATASET_PATH, {"n_train": len(splits["train"])}),
    }
